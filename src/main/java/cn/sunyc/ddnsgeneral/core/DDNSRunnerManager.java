...
public class DDNSRunnerManager implements ApplicationContextAware {
    ...
    public void refreshDDNSRunner(DDNSConfigDO ddnsConfigDO) {
        ...
        final DDNSRunner<?> ddnsRunner = runnerCacheMap.getOrDefault(uniqueKey, new DDNSRunner<>(this.applicationContext, localIpService, ddnsConfigRepository, msgNotifierConfigService));
        ...
        try {
            ...
            msgNotifierConfigService.sendMsg(MsgType.ERROR,new JSONObject()
                    .fluentPut("timeStr", new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date()))
                    .fluentPut("msg", e.getMessage()));
    ...
    public void runNow(DDNSConfigKey ddnsConfigKey) {
        ...
        final DDNSRunner<?> ddnsRunner = runnerCacheMap.get(uniqueKey);
        if (null == ddnsRunner) {
            final String errorMsg = "ddns runner not found. uniqueKey:" + uniqueKey;
            msgNotifierConfigService.sendMsg(MsgType.ERROR,new JSONObject()
                    .fluentPut("timeStr", new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date()))
                    .fluentPut("msg", errorMsg));
